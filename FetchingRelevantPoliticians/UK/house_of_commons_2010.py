...
# URL for the 2010 election
url_2010 = "https://en.wikipedia.org/wiki/List_of_MPs_elected_in_the_2010_United_Kingdom_general_election"
...
def fetch_mps_data(url):
    ...
    for row in rows:
        ...
        if len(cells) >= 5:  # Adjust based on the number of columns
            ...
            if mp_name_link and "wiki" in mp_name_link["href"] and "redlink=1" not in mp_name_link["href"]:
                mp_name = mp_name_link.get_text(strip=True)
                wiki_tag = mp_name_link["href"].split("/wiki/")[-1]

            if mp_name and wiki_tag:  # Only append if valid name and tag
                data.append({
...
def save_to_csv(data):
    ...
    file_path = os.path.join("election_data", "mps_2010.csv")
    df.to_csv(file_path, index=False, encoding="utf-8-sig")
    print(f"Data saved to {file_path}.")

def main():
    print("Processing data for 2010...")
    data = fetch_mps_data(url_2010)
    save_to_csv(data)
